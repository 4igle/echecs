...
class Pion(Piece):
    ...
    def deplacement_verif(self, pos1, pos2, passant, pieces, plateau, score, simulation):
        def changer_piece():
            if simulation:
        ...
        if pos1[0] == pos2[0] and plateau[pos2] is None:
            ...
            if int(pos2[1]) == int(pos1[1]) + sens:
                if self.coup_met_pas_en_echec(passant,
                                              plateau,
                                              pieces,
                                              score,
                                              pos1, pos2):
        ...
        # Prise (diagonale)
            ...
            # prise en passant
                ...
                if self.coup_met_pas_en_echec(passant, plateau_p, pieces_p, score, pos1, pos2):
...
class Plateau:
    ...
    def deplacement_possible(self, pos1, pos2, couleur):
    ...
    def partie_bot(self, couleur_jouee, difficulte):
        couleur = 'blanc'
        coups_jouables = self.coups_jouables(couleur)
        while not coups_jouables == []:
            self.show()
            sleep(0.2)
            ...
            if not couleur == couleur_jouee:
                pos1, pos2 = self.meilleur_coup(coups_jouables, couleur, couleur, difficulte, difficulte)
    ...
    def partie_bot_rapide(self, couleur_jouee, difficulte, fraction):
        couleur = 'blanc'
    ...
    def echec(self, couleur):
        ...
        for case_piece in self.pieces[couleur_adverse]:
            if self.deplacement_possible(case_piece, case_roi, couleur_adverse):
                return True
        return False

    def echec_et_mat(self, couleur):
        return self.coups_jouables(couleur) == [] and self.echec(couleur)

    def pat(self, couleur):
        return self.coups_jouables(couleur) == [] and not self.echec(couleur)

    def coups_jouables(self, couleur):
        coups_possibles = []
    ...
    def coups_jouables_dict(self, couleur):
        coups_possibles = dict()
        for coup in self.coups_jouables(couleur):
            if coup[0] in coups_possibles.keys():
                coups_possibles[coup[0]].add(coup[1])
            else:
                coups_possibles[coup[0]] = {coup[1]}
        return coups_possibles

    def meilleur_coup(self, coups_jouables, couleur, couleur_joueur, recursion_initiale, recursion):
        """
        ...
        if recursion != recursion_initiale:
            ...
            for coup in coups_jouables:
                ...
                mini, maxi = simulation.meilleur_coup(simulation.coups_jouables(couleur_adverse),
                                                      couleur_adverse,
                                                      couleur_joueur,
                                                      recursion_initiale,
                                                      recursion - 1)
    ...
    def meilleur_coup_rapide(self, coups_jouables, couleur, couleur_joueur, recursion_initiale, recursion, fraction):
        """
            Principe du meilleur coup : coup offrant le meilleur gain de score minimum assuré
            Si deux coups on le meilleure gain minimum assuré, on choisi celui avec le meilleur
            maximum probable.
        ...
        if recursion != recursion_initiale:
            stats_min, stats_max = 1000, -1000
            if len(coups_jouables[0])//fraction == 0:
                return -1000, -1000
            ...
            arret_index = (len(coups_verif) // fraction) - len(coups_prise)
            if arret_index < 0:
                arret_index = 0
            ...
            coups_verif = coups_prise + coups_non_prise[:arret_index]
